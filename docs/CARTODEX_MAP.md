---
last_mapped: 2026-06-25T18:48:40Z
total_files: 33
total_tokens: 28228
---
...
> Auto-generated by Cartodex, a Codex port inspired by Cartographer. Last mapped: 2026-06-25T18:48:40Z
...
| `README.md` | Public install, init, config, scanner, scout-agent, and mapping usage docs. | 1003 |
...
| File | Purpose | Tokens |
| --- | --- | ---: |
| `src/config.ts` | Loads defaults, parses config JSON, validates `ignore`, repo-relative Markdown `mapPath`, and `scoutAgent` model settings. | 1073 |
| `cartodex.config.json` | Current repo config: `mapPath` is `docs/CARTODEX_MAP.md`; `ignore` is empty; `scoutAgent` pins `gpt-5.3-codex-spark` with medium reasoning. | skipped by scanner default path ignore |
...
| `tests/init.test.ts` | Covers idempotency, default/custom map-path rendering, scout-agent rendering, stale/conflict handling, force, and check mode. | 3245 |
...
| File | Purpose | Tokens |
| --- | --- | ---: |
| `src/templates/skill/SKILL.md` | Packaged Cartodex skill workflow template with `{{mapPath}}` placeholders and concise completion-message guidance. | 1351 |
...
| `.agents/skills/cartodex/SKILL.md` | Installed local copy of the Cartodex skill used in this repo. | 1395 |
...
- Config loading is fail-fast and message-oriented. Unknown config fields are ignored, but `ignore` must be an array of strings, `mapPath` must be a repo-relative `.md` path without traversal, and `scoutAgent.model` / `scoutAgent.reasoningEffort` must be non-empty strings when present.
