...
> Auto-generated by Cartodex, a Codex port inspired by Cartographer. Last mapped: 2026-06-25T19:25:02Z
...
The current workspace includes update-mode changes that make the configured `mapPath` flow into both the managed `AGENTS.md` section and the installed Cartodex skill prompt. It also supports `scoutAgent` configuration, so `cartodex init` can render the configured scout model and reasoning effort into downstream `.codex/agents/cartodex-scout.toml` files. The scanner now reports directory token summaries, and the Cartodex skill requires generated map token cells to be reconciled against scanner JSON before writing.
...
| `README.md` | Public install, init, config, scanner, scout-agent, and mapping usage docs. | 1020 |
...
**Purpose**: Centralizes `cartodex.config.json` loading and validation so init, scout-agent rendering, and scanning share one configuration contract.
...
| File | Purpose | Tokens |
| --- | --- | ---: |
| `src/config.ts` | Loads defaults, parses config JSON with `unknown` narrowing, validates `ignore`, repo-relative Markdown `mapPath`, and `scoutAgent` model settings. | 1073 |
| `cartodex.config.json` | Current repo config: `mapPath` is `docs/CARTODEX_MAP.md`; `ignore` is empty; `scoutAgent` pins `gpt-5.3-codex-spark` with medium reasoning. | unresolved: default path ignored by scanner |
| `tests/config.test.ts` | Covers defaults, custom config merge, omitted fields, ignored extra fields, invalid map paths, and invalid scout-agent settings. | 856 |

**Exports**: `CARTODEX_CONFIG_FILE`, `DEFAULT_MAP_PATH`, `DEFAULT_SCOUT_AGENT_MODEL`, `DEFAULT_SCOUT_AGENT_REASONING_EFFORT`, `CartodexConfig`, `loadCartodexConfig`.

**Dependencies**: Node `fs` and `path` helpers.

**Dependents**: `src/init/init.ts` uses `mapPath` for AGENTS and skill prompt rendering and `scoutAgent` for scout TOML rendering; `src/scanner/ignore.ts` uses `ignore` patterns during scans.
...
| File | Purpose | Tokens |
| --- | --- | ---: |
| `src/init/init.ts` | Orchestrates git-root discovery, config loading, template rendering, file status analysis, writes, blocking rules, and user messages. | 1657 |
| `src/init/repo.ts` | Finds the nearest parent git root by checking `.git` files or directories. | 202 |
| `src/init/templates.ts` | Reads packaged templates, injects managed markers, renders map-path and scout-agent placeholders, and defines install targets. | 1615 |
| `src/templates/AGENTS.cartodex.md` | Template for the managed AGENTS marker block. | 96 |
| `tests/init.test.ts` | Covers idempotency, default/custom map-path rendering, scout-agent rendering, stale/conflict handling, force, and check mode. | 3385 |

**Exports**: `initCartodex`, `upsertAgentsSection`, `findGitRoot`, template constants, `INIT_TEMPLATES`, `renderInitTemplates`, `renderAgentsCartodexSection`, `renderCartodexSkillTemplate`, `renderCartodexScoutAgentTemplate`.
...
| File | Purpose | Tokens |
| --- | --- | ---: |
| `src/scanner/scan-codebase.ts` | Core traversal, text detection, skip taxonomy, directory token summaries, output formatters, and scanner CLI. | 3371 |
...
| `tests/scanner/scan-codebase.test.ts` | Behavioral coverage for scan results, directory summaries, skips, ignores, output formats, and parser errors. | 2010 |
...
| File | Purpose | Tokens |
| --- | --- | ---: |
| `src/templates/skill/SKILL.md` | Packaged Cartodex skill workflow template with `{{mapPath}}` placeholders, directory token-summary expectations, and token-cell reconciliation guidance. | 1531 |
| `src/templates/skill/resources/cartodex-map-structure.md` | Canonical generated map structure, including `total_files`, `total_tokens`, and token-cell resolution rules. | 392 |
| `src/templates/skill/resources/subagent-report-format.md` | Required subagent report format, including scanner-grounded token-cell guidance. | 332 |
| `src/templates/codex/cartodex-scout.toml` | Read-only scout agent template with `{{scoutAgentModel}}` and `{{scoutAgentReasoningEffort}}` placeholders. | 373 |
| `.agents/skills/cartodex/SKILL.md` | Installed local copy of the Cartodex skill used in this repo. | 1575 |
...
| File | Purpose | Tokens |
| --- | --- | ---: |
| `AGENTS.md` | Contributor and agent instructions, including generated-asset guidance and the Cartodex map pointer. | 772 |
...
4. `loadCartodexConfig` resolves the repo `mapPath`, scanner ignore config, and scout-agent model settings once for the run.
5. `renderInitTemplates({ mapPath, scoutAgentModel, scoutAgentReasoningEffort })` injects the concrete map path into the installed skill prompt and scout settings into `.codex/agents/cartodex-scout.toml`; `upsertAgentsSection` injects the map path into `AGENTS.md`.
...
7. Directory token totals are summarized from scanned files for every discovered directory.
8. Output is JSON `ScanResult`, tree view, or compact token-sorted listing.

`ScanResult` contains `root`, `files`, `directories`, `directory_summaries`, `total_tokens`, `total_files`, and `skipped`. Skipped reasons include `permission_denied`, `too_large`, `binary`, `too_many_tokens`, and `read_error: ...`.
...
- Config loading is fail-fast and message-oriented with `unknown` narrowing before assignment. Unknown config fields are ignored, but `ignore` must be an array of strings, `mapPath` must be a repo-relative `.md` path without traversal, and `scoutAgent.model` / `scoutAgent.reasoningEffort` must be non-empty strings when present.
- Cartodex-managed files use the `cartodex-managed` marker. The AGENTS block uses `<!-- CARTODEX:START -->` and `<!-- CARTODEX:END -->`.
- Template rendering uses `{{mapPath}}` placeholders for installed skill and AGENTS assets and `{{scoutAgentModel}}` / `{{scoutAgentReasoningEffort}}` for the scout agent TOML; default rendering falls back to `docs/CARTODEX_MAP.md`, `gpt-5.4-mini`, and `medium`.
...
- Generated map token columns should come from scanner `files[]`, `directory_summaries[]`, or `skipped[]`; unresolved rows must be called out in the completion message.
...
- Scout-agent config changes make `.codex/agents/cartodex-scout.toml` stale, but do not change the installed skill prompt or AGENTS block.
- The checked-in repo config intentionally pins the local scout agent to `gpt-5.3-codex-spark` with medium reasoning even though the code default is `gpt-5.4-mini`.
...
- `directory_summaries` aggregate only scanned files, so skipped files and ignored generated bundles do not contribute to directory token totals.
...
**To change scout-agent defaults or rendering**: Inspect `src/config.ts`, `src/init/templates.ts`, `src/templates/codex/cartodex-scout.toml`, `.codex/agents/cartodex-scout.toml`, `tests/config.test.ts`, and `tests/init.test.ts`.
...
**To change Cartodex token accounting guidance**: Inspect `src/scanner/scan-codebase.ts`, `src/templates/skill/SKILL.md`, `src/templates/skill/resources/cartodex-map-structure.md`, `src/templates/skill/resources/subagent-report-format.md`, and their installed `.agents/skills/cartodex/` counterparts after running init.
